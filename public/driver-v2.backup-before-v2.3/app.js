import{MapController}from'./map-controller.js?v=23';
import{GpsController}from'./gps-controller.js?v=23';
import{CameraController}from'./camera-controller.js?v=23';
import{VoiceController}from'./voice-controller.js?v=23';
...
async function load(){state.id=routeId();if(!state.id)throw new Error('Route ID is missing');mapCtl.init();gps=new GpsController(onGps,onGpsError);const r=await fetch(`/api/driver-v3/route/${encodeURIComponent(state.id)}`,{cache:'no-store'});const p=await r.json();if(!r.ok)throw new Error(p.error||'Could not load route');drawRoute(p.route||{},true);$('loading').classList.add('hidden');$('routeStatus').textContent='Route ready';updateButtons()}
