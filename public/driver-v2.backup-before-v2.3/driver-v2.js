...
function manoeuvreIcon(instruction){
  ...
  if(text.includes('arriv'))return 'â—';
  return 'â†‘';
}
...
function updateGuidance(progress,offRoute){
  ...
  el('roadStatus').textContent=ins.street||ins.roadNumbers?.join(' Â· ')||'Route';
  const mph=ins.speedLimit?.maxSpeedLimitMph;
  el('speedLimit').textContent=mph?`${mph} mph`:'Limit â€”';
...
async function reroute(){
  ...
  try{
    const response=await fetch(`/driver-v3/route/${encodeURIComponent(state.id)}/reroute`,{
...
function updateButtons(){
  el('gpsBtn').textContent=state.watchId===null?'ðŸ“':'ðŸ“¡';
  el('gpsBtn').classList.toggle('active',state.watchId!==null);
  el('voiceBtn').textContent=state.voice?'ðŸ”Š':'ðŸ”‡';
  el('voiceBtn').classList.toggle('active',state.voice);
  el('wakeBtn').classList.toggle('active',!!state.wakeLock);
  el('fullscreenBtn').textContent=document.body.classList.contains('fullscreen')?'â†™':'â›¶';
}

async function clearOldDriverCaches(){
  try{
    if('caches' in window){
      const keys=await caches.keys();
      await Promise.all(
        keys
          .filter((key)=>key.startsWith('coach-safe-shell-'))
    ...
    console.warn('Driver V2 cache cleanup skipped',error);
  }
}
...
async function load(){
  ...
  state.tileLayer=L.tileLayer('/driver-v2/tiles/{z}/{x}/{y}.png?v=22',{
    tileSize:256,
    zoomOffset:0,
    minZoom:2,
    maxZoom:20,
  ...
  state.tileLayer.on('tileload',()=>{
    ...
    if(loadedTileCount===1){
      el('routeStatus').textContent='Map ready';
      el('routeStatus').className='status good';
    }
  });

  state.tileLayer.on('tileerror',(event)=>{
    console.warn('Driver V2 HERE tile error',event?.tile?.src||event);
    el('routeStatus').textContent='Map tile error';
    el('routeStatus').className='status bad';
  });

  state.tileLayer.addTo(state.map);

  const response=await fetch(`/api/driver-v3/route/${encodeURIComponent(state.id)}`,{cache:'no-store'});
  const payload=await response.json();
...
el('reportBtn').addEventListener('click',()=>window.open(`/driver-v3/route/${encodeURIComponent(state.id)}#driverReportForm`,'_blank'));
