/* COACH_SAFE_VERIFIED_PINS_STAGE142A
   Hotfix: avoids MutationObserver/control creation feedback loops.
*/
(() => {
  ...
  function createControl(input) {
    ...
    control.innerHTML = `
      <button type="button" class="verified-pin-stage142-button">
        Verify pin
      </button>
      <span class="verified-pin-stage142-status unverified">
        Not verified
      </span>
    `;
  ...
  function scheduleRefresh() {
    ...
    refreshTimer = setTimeout(refreshInputs, 80);
  }
  ...
  /*
   * Observe ONLY child additions. The callback never writes immediately:
   * it schedules one debounced refresh. Because controls are kept in a
   * WeakMap, the refresh does not keep creating new DOM nodes.
   */
