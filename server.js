...
<div id="driverMapShell" class="driver-map-shell">
  ...
  <div id="mapInstructionOverlay" class="map-instruction-overlay"><strong id="mapNextTurn">Next turn loading…</strong><span id="mapNextDistance">—</span><small id="mapLaneSummary">Lane guidance appears when available</small></div>
...
function cleanSpeechText(text){return String(text||'Continue on the approved route').replace(/\s+/g,' ').replace(/M(\d+)/gi,'M $1').replace(/A(\d+)/gi,'A $1').trim()}
...
document.getElementById('mapFullscreenBtn')?.addEventListener('click',async()=>{const shell=document.getElementById('driverMapShell');const btn=document.getElementById('mapFullscreenBtn');if(!shell)return;const large=shell.classList.toggle('large-map-mode');document.body.style.overflow=large?'hidden':'';if(btn)btn.textContent=large?'Exit large map':'Large map';setTimeout(()=>{map.invalidateSize(true);if(currentGps){map.setView([currentGps.lat,currentGps.lng],Math.max(map.getZoom(),16),{animate:true})}else{fit()}},120);toast(large?'Large map mode on.':'Large map mode off.','success')});
