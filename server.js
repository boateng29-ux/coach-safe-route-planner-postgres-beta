...
<div id="driverMapShell" class="driver-map-shell">
  ...
  <div class="map-nav-overlay" aria-live="polite"><div class="label">Next turn</div><div id="mapNextTurn" class="turn">Start journey to load next instruction</div><div id="mapNextDistance" class="meta">—</div><div id="mapLaneHint" class="lane">Lane information appears when available</div></div>
  <div class="driver-map-controls" aria-label="Live map controls">
    ...
    <button id="mapFullscreenBtn" class="secondary" type="button">Large map</button>
...
function syncMapControlStates(){const wakeBtn=document.getElementById('mapWakeLockBtn');if(wakeBtn){wakeBtn.classList.toggle('active',!!wakeLock);wakeBtn.textContent=wakeLock?'Screen stays on':'Keep screen on'}const fullBtn=document.getElementById('mapFullscreenBtn');if(fullBtn){fullBtn.textContent=document.getElementById('driverMapShell')?.classList.contains('large-map-mode')?'Exit large map':'Large map'}updateRouteUpButton()}
...
function setAdvancedInstructionPanel(ins, progressM, distToNext) {
  ...
  const laneText = lane
    ...
    : 'Lane guidance not returned for this junction. Follow road signs.';
  ...
  let speedText = 'Speed limit not returned for this road section.';
  if (mph || kmh) {
    speedText = mph ? String(mph) + ' mph' : String(kmh) + ' km/h';
    if (mph && kmh) {
      speedText += ' / ' + String(kmh) + ' km/h';
    }
  }
  ...
  setText('navRoadInfo', roadBits.length ? roadBits.join(' · ') : '—');
  ...
  if (camera) {
    ...
    setText('navCameraAlert', (camera.type || 'Camera alert') + ' in ' + camDistance);
...
document.getElementById('mapFullscreenBtn')?.addEventListener('click',()=>{const shell=document.getElementById('driverMapShell');const btn=document.getElementById('mapFullscreenBtn');if(!shell)return;const large=shell.classList.toggle('large-map-mode');document.body.style.overflow=large?'hidden':'';if(btn)btn.textContent=large?'Exit large map':'Large map';setTimeout(()=>{map.invalidateSize(true);if(currentGps){map.setView([currentGps.lat,currentGps.lng],Math.max(map.getZoom(),16),{animate:true})}else{fit()}},150);toast(large?'Large map mode on.':'Large map mode off.','success')});
...
function toggleVoiceGuidance(){if(!speechAvailable()){toast('Spoken guidance is not supported in this browser.','error');setVoiceStatus('Not supported');return}voiceEnabled=!voiceEnabled;if(voiceEnabled&&!journeyStarted){journeyStarted=true;logEvent('Journey started for spoken guidance.');postJourneyEvent('journey_started','Driver started journey with map voice control.',currentGps?{lat:currentGps.lat,lng:currentGps.lng,accuracyM:Math.round(currentGps.accuracy||0)}:{})}setVoiceStatus(voiceEnabled?'On':'Off');if(voiceEnabled){toast('Voice guidance on. Listen for confirmation.','success');logEvent('Voice guidance on.');postJourneyEvent('voice_guidance_enabled','Driver enabled spoken turn-by-turn guidance from the live map.',{});const current=(instructions[currentInstructionIndex]||instructions[0]||{}).instruction||'Continue on route';speak('Voice guidance is on. '+current, true)}else{try{window.speechSynthesis.cancel()}catch(e){}toast('Voice guidance off.','success');logEvent('Voice guidance off.');postJourneyEvent('voice_guidance_disabled','Driver muted spoken turn-by-turn guidance from the live map.',{})}}
