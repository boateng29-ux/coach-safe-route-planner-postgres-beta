...
<main class="content">
  ...
  <section class="card nav-card"><h2>Spoken turn-by-turn</h2><p class="muted">Uses live GPS to speak the next route instruction. The visual instruction remains on screen as a backup only. This is beta guidance, not a certified satnav.</p><div class="nav-top"><div class="voice-status"><strong>Voice guidance</strong><span id="voiceStatus">Off</span></div><div class="nav-instruction"><span class="nav-label">Current spoken instruction</span><div id="navCurrentInstruction" class="nav-main">Start live GPS, then enable voice.</div><div class="nav-meta"><div class="gps-pill"><strong>Distance to next</strong><span id="navDistanceToNext">—</span></div><div class="gps-pill"><strong>Route progress</strong><span id="navProgressText">0%</span></div><div class="gps-pill"><strong>Remaining distance</strong><span id="navRemainingDistance">—</span></div><div class="gps-pill"><strong>ETA remaining</strong><span id="navEtaRemaining">—</span></div></div></div><div class="nav-bar" aria-label="Route progress"><div id="navProgressFill" class="nav-fill"></div></div><div class="buttons"><button id="journeyStartBtn" type="button">Start journey</button><button id="voiceGuidanceBtn" class="secondary" type="button">Enable voice</button><button id="nextInstructionBtn" class="secondary" type="button">Repeat instruction</button><button id="recalcBtn" class="secondary" type="button">Recalculate route</button><button id="journeyCompleteBtn" class="secondary" type="button">Complete route</button></div><div id="driverEventLog" class="event-log"><p>Journey event log ready.</p></div></div></section>
  ...
  <section class="card wide"><p class="muted"><strong>Important:</strong> Spoken turn-by-turn guidance is an aid only. Follow road signs, temporary restrictions, coach access signs and operator instructions at all times.</p></section>
</main>
...
function setVoiceStatus(text){setText('voiceStatus',text);const btn=document.getElementById('voiceGuidanceBtn');if(btn){btn.classList.toggle('active',voiceEnabled);btn.textContent=voiceEnabled?'Voice on':'Enable voice'}}
...
document.getElementById('voiceGuidanceBtn')?.addEventListener('click',()=>{if(!speechAvailable()){toast('Spoken guidance is not supported in this browser.','error');setVoiceStatus('Not supported');return}voiceEnabled=!voiceEnabled;setVoiceStatus(voiceEnabled?'On':'Off');if(voiceEnabled){toast('Spoken guidance enabled.','success');logEvent('Spoken guidance enabled.');postJourneyEvent('voice_guidance_enabled','Driver enabled spoken turn-by-turn guidance.',{});speakCurrentInstruction(true)}else{window.speechSynthesis.cancel();toast('Spoken guidance muted.','success');logEvent('Spoken guidance muted.');postJourneyEvent('voice_guidance_disabled','Driver muted spoken turn-by-turn guidance.',{})}});
