...
<script>window.ROUTE_EXPORT_DATA=${jsonForHtml(route)};window.DRIVER_ROUTE_ID=${JSON.stringify(record.id)};

// --- Driver controls reliability hotfix ---
function toggleDriverMapFullscreenFallback(btn) {
  const shell = document.getElementById('driverMapShell');
  if (!shell) return;
  const turningOn = !shell.classList.contains('large-map-mode') && !document.fullscreenElement;
  Promise.resolve().then(async function () {
    if (turningOn) {
      try { if (shell.requestFullscreen) await shell.requestFullscreen(); } catch (e) {}
      shell.classList.add('large-map-mode');
      document.body.style.overflow = 'hidden';
      if (btn) btn.textContent = 'Exit full screen';
      toast('Full screen map on.', 'success');
    } else {
      try { if (document.fullscreenElement && document.exitFullscreen) await document.exitFullscreen(); } catch (e) {}
      shell.classList.remove('large-map-mode');
      document.body.style.overflow = '';
      if (btn) btn.textContent = 'Full screen';
      toast('Full screen map off.', 'success');
    }
    setTimeout(function () {
      try {
        map.invalidateSize(true);
        if (currentGps) map.setView([currentGps.lat, currentGps.lng], Math.max(map.getZoom(), 16), { animate: true });
        else fit();
      } catch (e) {}
    }, 200);
  });
...
function repairDriverMapControls() {
  try {
    removeDirectionToggleArtifacts();
    const controls = document.querySelector('.driver-map-controls');
    if (controls) {
      controls.style.pointerEvents = 'auto';
      controls.style.zIndex = '5000';
...
repairDriverMapControls();
setTimeout(repairDriverMapControls, 400);
setTimeout(repairDriverMapControls, 1200);
setInterval(repairDriverMapControls, 3000);

document.addEventListener('click', function (event) {
  const btn = event.target.closest('#mapCenterBtn,#mapRecalcBtn,#mapFullscreenBtn,#mapWakeLockBtn,#mapVoiceBtn');
  if (!btn) return;
  event.preventDefault();
  event.stopPropagation();

  if (btn.id === 'mapCenterBtn') {
    if (currentGps) {
      followDriver = true;
      map.setView([currentGps.lat, currentGps.lng], Math.max(map.getZoom(), 16), { animate: true });
      toast('Map centred on your location.', 'success');
    } else {
      toast('Start GPS first.', 'error');
    }
    return;
  }

  if (btn.id === 'mapRecalcBtn') {
    recalcRouteFromGps(btn);
    return;
  }

  if (btn.id === 'mapFullscreenBtn') {
    toggleDriverMapFullscreenFallback(btn);
    return;
  }

  if (btn.id === 'mapWakeLockBtn') {
    toggleWakeLock();
    return;
  }

  if (btn.id === 'mapVoiceBtn') {
    toggleVoiceGuidance();
  }
}, true);
