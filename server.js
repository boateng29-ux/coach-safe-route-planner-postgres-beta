...
<main class="content">
  ...
  <section class="card wide"><p class="muted"><strong>Important:</strong> Live GPS mode is an aid only. It is not yet full turn-by-turn satnav. Follow road signs, temporary restrictions and operator instructions at all times.</p></section>
</main>
...
const data=window.ROUTE_EXPORT_DATA;const routePoints=(data.points||[]);const map=L.map('driverMap',{zoomControl:true,preferCanvas:true}).setView([51.5072,-0.1276],10);L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,attribution:'&copy; OpenStreetMap contributors',detectRetina:true,crossOrigin:true}).addTo(map);const pin=(c)=>L.divIcon({className:'',html:'<span class="coach-map-pin '+c+'"></span>',iconSize:[22,22],iconAnchor:[11,11],popupAnchor:[0,-12]});let routeLine=null;if(routePoints.length){routeLine=L.polyline(routePoints,{weight:7,opacity:.9,className:'coach-route-line'}).addTo(map);L.marker(routePoints[0],{icon:pin('start')}).bindPopup('Start: '+(data.origin?.label||'Start')).addTo(map);L.marker(routePoints[routePoints.length-1],{icon:pin('end')}).bindPopup('Destination: '+(data.destination?.label||'Destination')).addTo(map);function fit(){map.invalidateSize(true);map.fitBounds(routeLine.getBounds(),{padding:[34,34],maxZoom:15})}[250,800,1400].forEach((d)=>setTimeout(fit,d));window.addEventListener('resize',fit)}
let watchId=null,currentGps=null,driverMarker=null,accuracyCircle=null,followDriver=false;const gpsIcon=L.divIcon({className:'',html:'<span class="driver-location-dot"></span>',iconSize:[24,24],iconAnchor:[12,12]});
